# Docker/Rancher Machine driver for Proxmox VE (jrytio fork)
...
**This is a fork of the original driver with modified naming to allow installation alongside the original `pve-node-driver`.**
...
1. In `local` cluster explorer (i.e. the cluster where Rancher is deployed) go to `Apps > Repositories > Create` and create repository with following settings:
    ...
    * Name: `jrytio-pve-node-driver`
    * Description: `Node Driver for Proxmox Virtual Environment (jrytio fork)`
    * Target: `http(s) URL to an index generated by Helm`
    * Index URL: `https://tideline3d.github.io/docker-machine-driver-pve`
    * Authentication: `None`

1. Go to `Extensions > Available` and install `Proxmox VE Node Driver (jrytio)`

1. Go to `Cluster Management > Drivers > Node Drivers > jrytio-pve > Edit config` and add your Proxmox domain to `Whitelist Domains`

### Rancher in air-gapped environment

1. Download all required files on your jump machine:

    1. Set a variable with version to download (see [releases](https://github.com/tideline3d/docker-machine-driver-pve/releases) for available versions):

        ```sh
        export JRYTIO_PVE_NODE_DRIVER_VERSION="1.0.0"
        ```

    1. Add Helm repository:

        ```sh
        helm repo add jrytio-pve-node-driver https://tideline3d.github.io/docker-machine-driver-pve
        ```

    1. Download the Helm Chart:

        ```sh
        helm pull jrytio-pve-node-driver/jrytio-pve-node-driver --version "${JRYTIO_PVE_NODE_DRIVER_VERSION}"
        ```

    1. Download driver binary:

        ```sh
        wget $(helm show values --jsonpath "{.nodeDriver.url}" "./jrytio-pve-node-driver-${JRYTIO_PVE_NODE_DRIVER_VERSION}.tgz")
        ```
    ...
    1. Download UI Extension:
        ...
        && wget --directory-prefix=$PWD/ui-extension $(helm show values --jsonpath "{.uiPlugin.endpoint}" "./jrytio-pve-node-driver-${JRYTIO_PVE_NODE_DRIVER_VERSION}.tgz")/files.txt \
        && wget --directory-prefix=$PWD/ui-extension/plugin --base $(helm show values --jsonpath "{.uiPlugin.endpoint}" "./jrytio-pve-node-driver-${JRYTIO_PVE_NODE_DRIVER_VERSION}.tgz")/ -i $PWD/ui-extension/files.txt
        ```
...
Download `docker-machine-driver-jrytio-pve` from [latest release](https://github.com/tideline3d/docker-machine-driver-pve/releases/latest) and put it in your `PATH`.
...
This is a fork of the [original docker-machine-driver-pve](https://github.com/Stellatarum/docker-machine-driver-pve) by Stellatarum.

## Contact

For issues with this fork, please open an issue on this repository.  
For the upstream project: [contact@stellatarum.com](mailto:contact@stellatarum.com)
